...
async function formatProposals(epoch: number, proposals: any[]): Promise<Proposal[]> {
  const currentTotalStake = await getTokenTotalSupply(DEDS.addr);
  const initializeds = await Promise.all(proposals.map((p) => getIsInitialized(DEDS.addr, p.candidate)));
  const approves = await Promise.all(proposals.map((p) => getApproveFor(DEDS.addr, p.candidate)));
  const rejecteds = await Promise.all(proposals.map((p) => getRejectFor(DEDS.addr, p.candidate)));
  const supplyAts = await Promise.all(proposals.map(async (p) => {
    const at = (p.start + p.period - 1);
    if (epoch > at) {
      return await getTotalBondedAt(DEDS.addr, at);
    }
