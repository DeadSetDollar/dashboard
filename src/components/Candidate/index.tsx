  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      const [
        ...
        getRecordedVote(DEDS.addr, user, candidate),
      ...
      if (!isCancelled) {
        ...
        setUserStake(toTokenUnitsBN(userStakeStr, DEDS.decimals));
      }
    }
    updateUserInfo();
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      let [
        ...
        getApproveFor(DEDS.addr, candidate),
        getRejectFor(DEDS.addr, candidate),
        getTokenTotalSupply(DEDS.addr),
        getEpoch(DEDS.addr),
        getStartFor(DEDS.addr, candidate),
        getPeriodFor(DEDS.addr, candidate),
        getIsInitialized(DEDS.addr, candidate),
      ]);
      ...
      if (epochN > endsAfter) {
        totalStakeStr = await getTotalBondedAt(DEDS.addr, endsAfter);
      }

      if (!isCancelled) {
        setApproveFor(toTokenUnitsBN(approveForStr, DEDS.decimals));
        setRejectFor(toTokenUnitsBN(rejectForStr, DEDS.decimals));
        setTotalStake(toTokenUnitsBN(totalStakeStr, DEDS.decimals));
        setEpoch(epochN);
